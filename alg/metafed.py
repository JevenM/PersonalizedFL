...
class metafed(torch.nn.Module):
    def __init__(self, args):
        ...
        self.optimizers = [
            optim.SGD(params=self.client_model[idx].parameters(), lr=args.lr)
    ...
    def init_model_flag(self, train_loaders, val_loaders):
        ...
        optimizers = [
            optim.SGD(params=self.client_model[idx].parameters(), lr=self.args.lr)
            for idx in range(client_num)
        ]
        for idx in range(client_num):
            client_idx = idx
            model, train_loader, optimizer, tmodel, val_loader = (
                self.client_model[client_idx],
    ...
    def client_train(self, c_idx, dataloader, round):
        ...
        model, train_loader, optimizer, tmodel = (
            self.client_model[client_idx],
            dataloader,
            self.optimizers[client_idx],
    ...
    def personalization(self, c_idx, dataloader, val_loader):
        ...
        model, train_loader, optimizer, tmodel = (
            ...
            dataloader,
            self.optimizers[client_idx],
            copy.deepcopy(self.client_model[self.csort[-1]]),
        )
