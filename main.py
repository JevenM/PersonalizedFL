...
if __name__ == '__main__':
    ...
    for a_iter in range(start_iter, args.iters):
        ...
        if args.alg == 'metafed':
            ...
            if args.alg == 'fedlp':
                if a_iter < start_round:
                    # server aggregation
                    algclass.server_aggre()
                else:
                    assert len(val_acc_list) != 0; "should have values!"
                    # 1反比例权重计算
                    # epsilon = 1e-6  # 防止除零
                    ...
                    # 2指数变换权重计算
                    weight_list = np.exp(-np.array(val_acc_list))
                    # 对权重进行归一化，使其总和为1
                    weight_list = weight_list / np.sum(weight_list)
        ...
        best_vacc, best_tacc, best_changed, best_epoch_, val_acc_list = evalandprint(
    ...
    if args.alg == 'metafed':
        ...
        best_vacc, best_tacc, best_changed, best_epoch, _ = evalandprint(
            args, algclass, train_loaders, val_loaders, test_loaders, SAVE_PATH, best_vacc, best_tacc, a_iter, best_changed)
        if best_epoch_ > best_epoch:
            best_epoch = best_epoch_
